...
# =============================
print("="*60)
print("Algorithme Génétique pour CVRP")
print("="*60)

filename = input("\n1. Nom du fichier VRP: ").strip() or "A-n34-k5.vrp"
n_veh = input("2. Nombre de véhicules (Enter = auto): ").strip()
...
print(f"\n✓ Fichier chargé: {N} points, {n_vehicles} véhicules, capacité {capacity}")
...
def creer_population(taille):
    ...
    for _ in range(taille):
        perm = customers.copy()
        random.shuffle(perm)
        pop.append([depot] + perm + [depot])
    return popdef selection(pop):
...
def algorithme_genetique(pop_size=100, generations=500):
    ...
    for g in range(generations):
        ...
        if g % 100 == 0: print(f"  Génération {g}: {best_cost:.2f}")
...
print(f"Véhicules utilisés: {len(routes)}/{n_vehicles}")
if optimal_sol:
...
for i, r in enumerate(routes):
    ...
    print(f"  V{i+1}: {' → '.join(map(str,r))} | Dist:{d:.0f} | Charge:{load}/{capacity}")
